...
const UploadFile = () => {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const tx = await web3.eth.sendTransaction({
        from: account,
        to: '0x37E551eD0C989FE65d08651DB7181a938Be0A613', // Replace with your payment address
        value: feeInWei,
        gas: gasEstimate,
      });
  ...
  return (
    <div
      ...
      style={{
        background: "linear-gradient(135deg, #c3e0f7 25%, #e8f2fd 50%, #d8e4fa 75%)",
      }}
    >
      <div className="max-w-lg mx-auto p-8 bg-gray-200 rounded-xl shadow-lg space-y-6">
        <h2 className="text-3xl font-bold  text-black">Upload and Pay Fee</h2>
        <button
          onClick={connect}
          className="w-full py-2 px-4 text-white bg-blue-400 rounded-lg shadow-md hover:bg-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-75"
        >
          {isConnected ? `Connected: ${account}` : 'Connect MetaMask'}
        </button>
        <div className="space-y-4" >
          ...
          <button
            onClick={handlePaymentAndUpload}
            className={`w-full py-2 px-4 text-white rounded-lg shadow-md transition-colors ${loading ? 'bg-gray-400' : 'bg-blue-500 hover:bg-blue-600'}`}
            disabled={loading}
          >
            {loading ? `Uploading... ${uploadProgress}%` : 'Upload and Pay Fee ▲'}
          </button>
